...
int serialPort = 1;       //Index of serial port that the Arduino is connected to
Serial myPort;
MultiTouchKit mtk;

int maxInputRange = 100;  // set the brightness of touch points (blobs)
...
void draw(){
  ...
  rawvalues = mtk.getValues();                                 // values used for visualization
  ...
  */
  ...
  // -------------- Uncomment this part, if you want to save the values in a CSV file --------------
//  values = mtk.getAdjustedValues();
//  rawvalues = mtk.getValues();
